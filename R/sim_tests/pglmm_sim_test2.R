...
if (n.reps==999) { # 999 is a flag for unbalanced design
...
res_mcmc <- run_model_safely({
  MCMCglmm(yi ~ x,
           ...
           prior = prior,
           data = dat,
           nitt = 53000, # increase default by x5
           burnin = 3000, thin = 10)
})
...
conv_inla <- !is.null(model_inla)  #theres no convergence metric so put a check that it ran at least



# save all models objects in RDATA
