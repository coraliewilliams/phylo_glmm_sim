...
# Check convergence %
# table(res1.dat$convergence, res1.dat$model, res1.dat$species_size)
# table(res2.dat$convergence, res2.dat$model)

#### Set specific convergence check for INLA models if s2_phylo are > 4 --> alternatively change to 3
res1.dat <- res1.dat |> 
  mutate(convergence = ifelse(model == "INLA" & (s2_phylo > 4), FALSE, convergence))
...
table(res1$species_size)/4/8000
table(res2$species_size)/3/1000
...
res1$model <- factor(res1$model, 
                     levels=c("brms", "MCMCglmm", "INLA", "glmmTMB"))
...
ggsave(filename = "output/SFigures/Figure_runtime_all_extra.png", width = 10, height = 8)
...
ggsave(filename = "output/SFigures/Figure_log_runtime_perspecies_extra.png", width = 10, height = 6)
