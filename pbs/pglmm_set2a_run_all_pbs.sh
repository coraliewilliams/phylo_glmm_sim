#!/usr/bin/env bash
# submit_all_pbs.sh
# Generate and qsub 20 PBS array jobs to cover 1–192,000 in chunks of 10,000

# (1) adjust this to your actual working directory:
WORKDIR="/srv/scratch/z5394590/pglmm/set2/"

# (2) create a place to store all the little PBS scripts
PBS_DIR="${WORKDIR}/pbs_scripts"
mkdir -p "${PBS_DIR}"

# (3) for each of the 20 blocks:
for (( chunk=0; chunk<20; chunk++ )); do
  start=$(( chunk*10000 + 1 ))
  end=$(( start + 10000 - 1 ))
  jobname="pglmm_set1a_${start}_${end}"
