...
For any comments or questions please contact Coralie Williams^[coralie.williams@unsw.edu.au] (corresponding author).
...
We specify the model to fit repeated measures per species as:

$$
y_{ij} = \beta_0 + \beta_1 x_{ij} + n_i + p_i + \varepsilon_{ij}
$$

Where:

- \( y_{ij} \): response for observation \( j \) in species \( i \)
- \( \beta_0, \beta_1 \): fixed effects (intercept and slope)
- \( n_i \sim \mathcal{N}(0, \sigma^2_{\text{species}}) \): random species effect (non-phylogenetic)
- \( p_i \sim \mathcal{N}(0, \sigma^2_{\text{phylo}} \mathbf{A}) \): phylogenetic effect, with \( \mathbf{A} \) the phylogenetic relatedness matrix
...
The model specification for one measure per species is:
...
# one measure per species
model_glmmTMB_simp <- glmmTMB(yi ~ x + propto(0 + species|g, phylo.mat),
                         data = dat,
                         REML = TRUE)
...
## Case study 2: Hydraulic plant data 
