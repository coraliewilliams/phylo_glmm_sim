...
hydra.dat$genus.Rand <- hydra.dat$genus
hydra.dat$animal <- hydra.dat$genus
...
# normal
time_norm <- system.time(
  m1 <- glmmTMB(log(Ks) ~ body_region * sex + (1|species) + propto(0 + species|g,phylo.mat),
                      family = gaussian(),
                      data = hydra.dat) # don't use REML (to get AIC)
  )
