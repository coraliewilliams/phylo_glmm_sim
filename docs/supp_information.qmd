...
pacman::p_load(
  ...
  knitr, bayestestR, performance, rbenchmark, here, DHARMa
)
...
kable(res_rand, 
  ...
  col.names = c("Model", "Species size", "Sample size", "Run time (s)", "Phylo variance", "Non-phylo variance", "Residual variance"),
  digits = 3,
  format = "html"
)
...
# check missing values ---> there is several missing values in the dataset
...
Now we want to create a wavelength dataset using the `pavo` package using the spectral reflectance data.
...
plot(Aplonis_metallica_a.Male.wing_cov ~ wl, type = "l", data = specs)


# use procspec to adjust negative values (maybe due to darker colors) by shifting them
...
Using the wavelength dataset we can now obtain spectral shape descriptors. These descriptors will be used in the model. Here we will focus on four descriptors of interest: brightness (B1), spectral slope (S1), spectral curvature (S9), and hue (H4).
...
# 4) Set thresholds for carotenoid range and saturation
h_lower <- 340   # start of red
h_upper <- 100   # end of yellow
C_min   <- 15 # min chroma
L_min   <- 20 # avoid very dark samples

# 5) Hue range test with wrap around
in_range <- if (h_lower <= h_upper) {
  h >= h_lower & h <= h_upper
} else {
  h >= h_lower | h <= h_upper
}

# 6) COmpute binary trait:  1 = carotenoid like present, 0 = absent
...
# get carotenoid dataset combined by body regions
# carotenoid: 1 = carotenoid like present in at least one body region, 0 = absent in all
carot.dat <- carot.dat.all |> 
  group_by(species, sex, indiv_rep) |> 
  summarise(carotenoid = as.integer(any(carotenoid==1)))

table(carot.dat$carotenoid)
...
We also want to create a dataset for the ordinal trait (proportion of body region with color) to understand if there is a relationship between the proportion of body region with carotenoid color presence.

```{r}
carot_ordinal <- carot.dat.all |>
  group_by(species, sex, indiv_rep) |>
  summarise(prop_carotenoid = mean(carotenoid), .groups = "drop")

# save as csv
write.csv(carot_ordinal, file="data/ordinal_data_for_model.csv", row.names = FALSE)
...
vc <- VarCorr(m2)
vc_cond <- subset(vc, component == "cond" & var1 == "(Intercept)" & is.na(var2))

phylo_name    <- "phylo"    # phylogenetic 
nonphylo_name <- "species"  # non-phylogenetic 

sigma2_p <- vc_cond$vcov[match(phylo_name, vc_cond$grp)] # phylogenetic variance estimate
sigma2_s <- vc_cond$vcov[match(nonphylo_name, vc_cond$grp)] # non-phylogenetic variance
...
On the log mean scale of the Gamma model, the phylogenetic species effect explained xxx% of the total species level random effect variance. This represents the degree of phylogenetic signal in the overall variance sourced from species.


Pairwise comparisons of body region brightness vs sex (add more description/explanation).
...
ggplot(aes(x = body_region, y = ratio, color = body_region)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.2) +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "grey40") +
  labs(x = "Body Region", y = "Female / Male Ratio", 
       title = "Brightness") +
  theme_bw() +
...
Here we want to model the absence or presence of carotenoid across all body regions accounting for sex and including species- and phylogeny-level random effects.

##### Modelling carotenoid presence

We will fit two models:
-   Binomial model with a logit link function
-   Zero-inflated binomial model with a logit link function
...
time_binom <- system.time(
  m3 <- glmmTMB(carotenoid ~ sex + + (1|species) + propto(0 + species|g,phylo.mat),
                family = binomial(link = "logit"),
                data = carot.dat)
)

# zero-inflated binomial
time_zbinom <- system.time(
  m4 <- glmmTMB(carotenoid ~ sex + + (1|species) + propto(0 + species|g,phylo.mat),
                ziformula = ~1,
                family = binomial(link = "logit"),
                data = carot.dat)
)
...
carotmod_output <- data.frame(
  model = c("Binomial", "Zero-inflated Binomial"),
  convergence = c(m3$sdr$pdHess, m4$sdr$pdHess),
  runtime = c(time_binom[["elapsed"]], time_zbinom[["elapsed"]]),
  AIC = c(AIC(m3), AIC(m4))
)
...
```{r}
#| cache: true 
res_zbinom <- DHARMa::simulateResiduals(fittedModel = m4)
plot(res_zbinom)

```


We found that the xxx shows improve model fit as the residual plots.
...
##### Modelling carotenoid percentage
...
hist(hydra.dat$Hv, breaks = 50, main = "Distribution of Hv", xlab = "Hv")
...
# Load bird tree (consensus tree = "combined tree")
plant.tree <- ape::read.tree(here("data/genus-level_phylogeny.tre"))
# plot tree
