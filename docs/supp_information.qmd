...
k.species <- 100   # number of species
...
set.seed(seed); u.s <- rnorm(k.species, 0, sqrt(sigma2.n))[sp.id]
set.seed(seed); u.p <- MASS::mvrnorm(1, mu=rep(0, k.species), Sigma=sigma2.p*phylo.mat)[sp.id]
set.seed(seed); ei <- rnorm(n, 0, sqrt(sigma2.e))
...
To fit a PGLMM using `glmmTMB` R package we specify the species-level random effect with phylogenetic relatedness using the `propto` covariance structure (McGillycuddy, 2023; Kristensen & McGillycuddy, 2025).

The `propto(0 + species| g, phylo.mat)` covariance structure has three notable components:

(1) To the left of the bar, we specify the random intercept at the species level (`0 + species`). It is very important to add `0 +` to appropriately model a multivariate species random effect. See explanations as to why this is necessary here:  [https://cran.r-project.org/web/packages/glmmTMB/vignettes/covstruct.html#construction-of-structured-covariance-matrices](https://cran.r-project.org/web/packages/glmmTMB/vignettes/covstruct.html#construction-of-structured-covariance-matrices).

(2) To the right of the bar, we first need to specify a factor variable, here denoted `g`, whose levels correspond to independent random‐effects vectors with shared covariance parameters. For PGLMM, it is specified as a single level for all observations (i.e. `dat$g <- 1`), so that a single random‐effect is fitted to the entire dataset. 

(3) After the comma on the right side of the bar, we specify the phylogenetic *correlation* matrix `phylo.mat`. We use a correlation matrix rather than a covariance matrix to avoid estimating a separate variance component per species (which would arise from each species variances). Instead, the standardized correlation matrix (with 1s on the diagonal) allows us to estimate a single phylogenetic variance component $\sigma^2_{p}$ that captures the overall magnitude of phylogenetic signal.
...
Obtain the random effect and residual variance estimates. Note that the phylogenetic variance estimate from `phyr` needs to be scaled by a factor to obtain the correct variance estimate.

```{r}
# get scaling factor for phylogenetic variance component
V <- phylo.mat 
V_scaled <- V / max(V)
scale_factor <- max(V) * exp(determinant(V_scaled)$modulus[1] / nrow(V))

# get phyr random effect variance estimates 
var_re_phyr <- c(as.numeric(model_phyr$s2r[2]/ scale_factor), #phylogenetic 
                 as.numeric(model_phyr$s2r[1]), #non-phylogenetic 
                 as.numeric(model_phyr$s2resid)) #residual 
...
phylo.prec.mat <- MCMCglmm::inverseA(tree)$Ainv
...
time.mcmc <- system.time({
  model_mcmc <- MCMCglmm(yi ~ x,
                         ...
                         data = dat,
                         verbose = FALSE,
                         nitt = 103000, # increase default by x10
                         burnin = 3000, # default
                         thin = 1) 
})
...
As there is only one observation per species, we cannot fit a non-phylogenetic effect, as it won't be separately identifiable from the phylogenetic effect. Therefore, we only include the phylogenetic random effect in the model (see Equation 3 in the manuscript). The model is specified as follows:
...
# Formula:          yi_mean ~ x_mean + propto(0 + species | g, phylo.mat)
# Data: dat.m
# 
#       AIC       BIC    logLik -2*log(L)  df.resid 
#     171.0     181.4     -81.5     163.0        96 
# 
# Random effects:
# 
# Conditional model:
#  Groups   Name      Variance Std.Dev. Corr                                            
#  g        species1  0.0855   0.292                                                    
#           species2  0.0855   0.292    0.87                                            
#           species3  0.0855   0.292    0.87 0.96                                       
#           species4  0.0855   0.292    0.00 0.00 0.00                                  
#           species5  0.0855   0.292    0.67 0.67 0.67 0.00                             
#           species6  0.0855   0.292    0.00 0.00 0.00 0.70 0.00                        
#           species7  0.0855   0.292    0.47 0.47 0.47 0.00 0.47 0.00                   
#           species8  0.0855   0.292    0.31 0.31 0.31 0.00 0.31 0.00 0.31              
#           species9  0.0855   0.292    0.80 0.80 0.80 0.00 0.67 0.00 0.47 0.31         
#           species10 0.0855   0.292    0.31 0.31 0.31 0.00 0.31 0.00 0.31 0.97 0.31    
#           ...       ...      ...      ...  ...  ...  ...  ...  ...  ...  ...  ...  ...
#  Residual           0.2528   0.503                                                    
# Number of obs: 100, groups:  g, 1
# 
# Dispersion estimate for gaussian family (sigma^2): 0.253 
# 
# Conditional model:
#             Estimate Std. Error z value Pr(>|z|)    
# (Intercept)   1.8540     0.8537    2.17     0.03 *  
# x_mean        1.4596     0.0558   26.14   <2e-16 ***
# ---
...
There is evidence of a positive association between `x` and the response, with an estimated effect of 1.46 (95% CI: 1.35–1.57). The phylogenetic variance component estimate is 0.08 and the residual variance estimate is 0.25. Hence the proportion of total variance explained by evolutionary relationships (i.e. phylogenetic signal) is $0.0855/(0.2528+0.0855) = 0.25$.  
...
bird_data <- read_delim("data/Spec_IndivReg.csv", 
...
#| echo: FALSE
# Get model info  ---------------------------
# check whether model has to be positive definite hessian
