...
![Image credits:](bird_color.png)

<span style="color:grey; font-size:0.9em;">Image credits: Andy Chilton, Boris Smokrovic, David Clode, Jaime Spaniol, Joshua J. Cotte, Vincent Van Zalinge.</span>
...
# residual of gaussian model
m1_res <- sigma(m1)

# residual of gamma model
m2_res <- trigamma(exp(unname(m2$fit$par["betadisp"])) )

# print
data.frame(
  model = c("Gaussian (log)", "Gamma"),
  residual_variance = c(m1_res^2, m2_res)
)
...
sigma2_s <- m5_re$Estimate[1]^2 # non-phylogenetic variance
sigma2_p <- m5_re$Estimate[2]^2 # phylogenetic variance estimate
...
![Image credits:](plant_trait.jpeg)
